    ...
    const bmList = bms?.map(el => {
        ...
        if ((!props.archive && !el.archived) || (props.archive && el.archived)) {
            ...
            <tr>
                ...
                <td>
                    <div className='bm-table-copy'>{el.link1 ?
                        <div className='bm-table-copy-icon' data-tip='Copied' id={el.link1}><Clipboard/></div> :
                        <div className='accounts-table-center'><Cross/></div>}</div>
                </td>
                <td className='link2'>
                    <div className='bm-table-copy'>{el.link2 ?
                        <div className='bm-table-copy-icon' data-tip='Copied' id={el.link2}><Clipboard/></div> :
                        <div className='accounts-table-center'><Cross/></div>}</div>
                </td>
                <td className='link3'>
                    <div className='bm-table-copy'>{el.link3 ?
                        <div className='bm-table-copy-icon' data-tip='Copied' id={el.link3}><Clipboard/></div> :
                        <div className='accounts-table-center'><Cross/></div>}</div>
                </td>
                {props.user.admin && <>
                    {country ? <td className='white'>{el.account ? `${el.account.id} ${country}` :
                            <div className='accounts-table-center'><Cross/></div>}</td> :
                        <td className='white'>{el.account ? el.account.id :
                            <div className='accounts-table-center'><Cross/></div>}</td>}
                </>
                }
            {!props.user.admin && !props.archive && <td className='white'>
                    <div className='bm-table-edit'>
                        <button data-id={el.id} data-name='b' disabled={el.statusId === 3}
                                onClick={handleProblemClick}><Error/></button>
                    </div>
                </td>}
                <td>
                    {props.user.admin ? props.user.permission.acc_bm_update &&
                        ...
                        </NavLink></div> : <div className='bm-table-edit'>
                        <button data-id={el.id}
                                onClick={handleArchiveClick}><Folder/></button>
                    </div>}
                </td>
            </tr>
    ...
    return (<div className='bm'>
        ...
        <table className='bm-table'>
            ...
            <tr>
                ...
                {props.user.admin && <th>Account</th>}
        ...
        {!props.user.admin && modalState && <div className='modal'>
            <div className='modal-window'>
                ...
                <div className='modal-window-data'>
                    {props.archive ? 'Do you really want to remove that BM from archive?' : 'Do you really want to move that BM to archive?'}
                </div>
                <div className='modal-window-yes-no'>
