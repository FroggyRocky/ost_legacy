   ...
   if(attribute===null || attribute == 'modal') {
    ...
    const accountsList = accounts?.map((el) => {
        ...
        if ((!props.archive && !el.archived) || (props.archive && el.archived)) {
            ...
            <tr>
                <td>
                    <div className='accounts-table-id'>
                        <div
                            className={`accounts-table-id-status ${TableAdditionalInfo.colorOfStatus(el.statusId)}`}></div>
                        <div className='accounts-table-id-click' onClick={TableAdditionalInfo.handleClick}
                             data-id={el.id}>
                            <Down/>
                        </div>
                ...
                {props.user.admin && <td className='create-date'>
                    <div className='accounts-table-date'>
                        <div className='accounts-table-date-time'>
                    ...
                    <div className='accounts-table-limited'>
                        ...
                        <div data-proxy_id={el.proxy_id} className='accounts-table-limited-traffic'>
                            ...
                            {el.proxy_traffic_total && el.proxy_traffic_left >= 0 ?
                                ...
                                </div> : <div className='accounts-table-limited-icon' onClick={getTraffic}
                                >
                                    <div className='accounts-table-bubble refresh-bubble'>Refresh</div>
                                    <Refresh className='account-refresh-icon'/>
                                </div>}
                ...
                {props.user.admin && <td className='accounts-table-bm-id'>
                    {el.bmId ? <div className='accounts-table-bm'>
                        <div
                            className={`accounts-table-bm-label ${TableAdditionalInfo.colorOfStatus(el.bm.statusId)}`}></div>
                        <div className='accounts-table-bm-icon'>{el.bm.statusId === 3 ?
                            <Error className='accounts-table-bm-icon-error'/> : <Tick className='icon-green'/>}
                        </div>
                    </div> : !props.archive && !props.user.admin && props.freeUserBms.length !== 0 ? <>
                        <select
                            name='bmId'
                ...
                {props.user.admin && props.user.permission.acc_bm === 2 && props.user.permission.acc_bm_update &&
                    <td className='multi'>
                        {el.proxy_ip && el.proxy_login && el.proxy_password ? <div className='accounts-table-multi'>
                            <div className='accounts-table-multi-icon' data-name={accountName}
                                 onClick={handleMultiClick}>
                                <Plus className={el.uuid && 'accounts-table-multi-success'}/>
                            </div>
                        </div> : <div className='accounts-table-center'><Error/></div>}
                    </td>}
                <td>
                    {props.user.admin ? props.user.permission.acc_bm_update &&
                        <div className='accounts-table-edit'><NavLink to={`/dashboard/adminacclist/edit/${el.id}`}>
                            <div><Pencil/></div>
                        </NavLink></div> : <div className='accounts-table-edit'>
            ...
            <tr id={el.id} hidden>
                <td colSpan={props.user.admin ? (props.user.permission.acc_bm === 2 && props.user.permission.acc_bm_update ? 10 : 9) : 7}
                    className='accounts-table-info-td'>
                    <div className='accounts-table-info-tr'>
                        ...
                        <div className='accounts-table-info-tr-info'>
                            ...
                            <div className='accounts-table-info-tr-info-tr'>
                                ...
                                <div className='accounts-table-info-tr-info-item'>
                                    <div>Password</div>
                                    {el.proxy_password ? <span className='proxy_password'>{el.proxy_password}</span> :
                                        <Cross/>}
                                </div>
                            </div>
                        </div>
                    </div>
                    {el.bmId && <div className='accounts-table-info-tr'>
                        <div className='accounts-table-info-tr-icon'>
                            {el.bm.statusId === 3 ? <>
                                ...
                                {!props.user.admin && !props.archive &&
                                    <div data-id={el.bm.id} data-name='b' onClick={handleProblemClick}>
                                        <Error/>
                                    </div>}
                            </>}
                        </div>
                        <div className='accounts-table-info-tr-info'>
                            <div className='accounts-table-info-tr-info-tr'>
                                ...
                                <div className='accounts-table-info-tr-info-item'>
                                    <div>Type</div>
                                    {el.bm.bmTypeId ? TableAdditionalInfo.getValueById(props.bmTypes, el.bm.bmTypeId) :
                                        <Cross/>}
                                </div>
                                <div className='accounts-table-info-tr-info-item'>
                                    <div>ID</div>
                            ...
                            <div className='accounts-table-info-tr-info-tr'>
                                ...
                                <div className='accounts-table-info-tr-info-copy'>
                                    <div>Link1</div>
                                    {el.bm.link1 ? <span className='copy' data-tip='Copied'
                                                         id={el.bm.link1}><Clipboard/></span> : <span><Cross/></span>}
                                </div>
                                <div className='accounts-table-info-tr-info-copy'>
                                    <div>Link2</div>
                                    {el.bm.link2 ? <span className='copy' data-tip='Copied'
                                                         id={el.bm.link2}><Clipboard/></span> : <span><Cross/></span>}
                                </div>
                                <div className='accounts-table-info-tr-info-copy'>
                                    <div>Link3</div>
                                    {el.bm.link3 ? <span className='copy' data-tip='Copied'
                                                         id={el.bm.link3}><Clipboard/></span> : <span><Cross/></span>}
                                </div>
                            </div>
                        </div>
                    </div>}
    ...
    function handleProblemClick(event) {
        ...
        window.addEventListener('keydown', (event) => {
            if (event.keyCode === 27) handleProblemModalNoClick()
        });
    ...
    const instance = axios.create({
        baseURL: "http://localhost:35000/api/v2", withCredentials: true, headers: {
    ...
    async function handleMultiClick(event) {
        ...
        let data = {
            ...
            'extensions': {
                'enable': true, 'names': ['editthiscookie.crx']
            },
        };
    ...
    function handleProblemModalYesClick() {
        async function sendProblem() {
            ...
            setModalAddTicketState({
                active: true, title: `${dataState.type === 'a' ? 'Account' : 'BM'}: ${dataState.id} ${countryName}`
            })
        }
    ...
    function handleArchiveClick(event) {
        ...
        window.addEventListener('keydown', (event) => {
            if (event.keyCode === 27) handleModalNoClick()
        });
    ...
    function handleAddTrafficClick(event) {
        ...
        window.addEventListener('keydown', (event) => {
            if (event.keyCode === 27) handleModalAddTrafficNoClick()
        });
    }
    ...
    async function handleModalAddTrafficYesClick() {
        if (addTrafficState.id && addTrafficState.proxy_id) {
            ...
            const data = {
                id: addTrafficState.id, proxy_id: addTrafficState.proxy_id, trafficAmount: dropDownState
            }
    ...
    return (<div className='accounts'>
        ...
        {!props.user.admin && modalArchiveState && <div className='modal'>
            <div className='modal-window'>
                ...
                <div className='modal-window-data'>
                    {props.archive ? 'Do you want to activate this account?' : 'Do you want to archive the account?'}
                </div>
                <div className='modal-window-yes-no'>
